...
/// EXECUTABLE NAME                   : Exe_elasticbandsmoother
...
exe_elasticbandsmoother::aa::Swc_elasticbandsmoother* g_swcSwc_elasticbandsmoother{nullptr};
 
static void SignalHandler(std::int32_t signal)
{
    if (signal == SIGTERM || signal == SIGINT)
    {
        g_swcSwc_elasticbandsmoother->Terminate();
    }
}
...
int main(int argc, char *argv[], char* envp[])
{
    ...
    if (araInitialized)
    {
        ara::log::Logger& appLogger{ara::log::CreateLogger("EBST", "Exe_elasticbandsmoother's main function")};
        ...
        exe_elasticbandsmoother::aa::Swc_elasticbandsmoother swcSwc_elasticbandsmoother;
        g_swcSwc_elasticbandsmoother = &swcSwc_elasticbandsmoother;
        
        // initialize software component
        proceed = swcSwc_elasticbandsmoother.Initialize();
        
        if (proceed)
        {
            // start software component
            swcSwc_elasticbandsmoother.Start();
            ...
            if (exec.HasValue())
            {
                ...
                swcSwc_elasticbandsmoother.Run();
            }
            else
            {
