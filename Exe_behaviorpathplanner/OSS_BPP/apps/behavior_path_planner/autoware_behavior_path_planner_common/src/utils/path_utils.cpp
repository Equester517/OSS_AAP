...
#include "rosless/utils/bpp_path_utils.hpp"
#include "rosless/utils/resample.hpp"
...
std::vector<int64_t> mergeLaneIdsPreservingOrder(
  ...
  const std::shared_ptr<const PlannerData> & planner_data)
{
  ...
  auto clipped_s_vec = calcPathArcLengthArray(reference_path, 0, reference_path.points.size(), 0.0);
  double clipped_goal_s = clipped_s_vec.empty() ? 0.0 : clipped_s_vec.back();
  snapPathEndToGoal(reference_path, goal_pose, clipped_goal_s);
