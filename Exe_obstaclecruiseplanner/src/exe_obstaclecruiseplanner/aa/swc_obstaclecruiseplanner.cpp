...
Swc_obstaclecruiseplanner::Swc_obstaclecruiseplanner()
    : m_logger(ara::log::CreateLogger("OCPL", "SWC", ara::log::LogLevel::kVerbose))
    , m_running{false}
    , m_workers(49)
    , m_current_scenario_id{0}
    , m_trajectory_buffer(std::make_unique<TrajectoryBuffer>())
{
}
...
void Swc_obstaclecruiseplanner::Run()
{
    ...
    // m_workers.Async([this] { m_RPort_MVP2OCP->ReceiveEventplanning_scenarioplanning_lanedriving_motionplanning_motionvelocityplanner_trajectoryCyclic(); });
    ...
    m_workers.Async([this] { m_RPort_SCr2OCP->ReceiveEventscenarioCyclic(); });
        ...
        for (std::size_t index = 0; index < lines.size(); ++index) {
            try {
                ...
                if (index + 1 < lines.size()) {
                    const int delay_ms = m_interval_dist(m_rng);
